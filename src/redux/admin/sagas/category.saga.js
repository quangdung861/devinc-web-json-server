...
function* createCategorySaga(action) {
  try {
    ...
    const result = yield axios.post(`https://devinc-web-json-server-i27x.vercel.app/categories`, {
...
function* updateCategorySaga(action) {
  try {
    ...
    yield axios.patch(`https://devinc-web-json-server-i27x.vercel.app/categories/${categoryId}`, {
      name: categoryName,
    });
    yield put({
...
function* deleteCategorySaga(action) {
  try {
    const values = action.payload;
    yield axios.delete(`https://devinc-web-json-server-i27x.vercel.app/categories/${values}`);
    yield put({
      type: REQUEST(CATEGORY_ADMIN_ACTION.GET_CATEGORY_LIST),
    });
