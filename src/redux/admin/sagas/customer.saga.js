...
function* getCustomerListSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/users`, {
      params: {
        _page: params.page,
        _limit: params.limit,
        _sort: "id",
...
function* deleteCustomerAdminSaga(action) {
  try {
    ...
    yield axios.delete(`https://devinc-web-json-server-i27x.vercel.app/users/${id}`);
