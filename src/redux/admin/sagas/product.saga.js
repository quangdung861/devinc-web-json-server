...
function* getProductListAdminSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/products`, {
      params: {
        _expand: "category",
        _page: params.page,
        _limit: params.limit,
...
function* getProductDetailSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/products/${id}`, {
      params: {
        _expand: "category",
...
function* createProductAdminSaga(action) {
  try {
    ...
    const result = yield axios.post(`https://devinc-web-json-server-i27x.vercel.app/products`, values);
    if (options) {
      for (let i = 0; i < options.length; i++) {
        yield axios.post(`https://devinc-web-json-server-i27x.vercel.app/options`, {
    ...
    if (images) {
      for (let i = 0; i < images.length; i++) {
        yield axios.post(`https://devinc-web-json-server-i27x.vercel.app/images`, {
          productId: result.data.id,
          url: images[i].url,
          name: images[i].name,
          type: images[i].type,
...
function* deleteProductAdminSaga(action) {
  try {
    const id = action.payload;
    yield axios.delete(`https://devinc-web-json-server-i27x.vercel.app/products/${id}`);
...
function* updateProductAdminSaga(action) {
  try {
    ...
    const result = yield axios.patch(
      `https://devinc-web-json-server-i27x.vercel.app/products/${id}`,
      values
    );
    // Option
    for (let i = 0; i < options.length; i++) {
      if (options[i].id) {
        yield axios.patch(`https://devinc-web-json-server-i27x.vercel.app/options/${options[i].id}`, {
          productId: result.data.id,
          name: options[i].name,
          bonusPrice: options[i].bonusPrice,
        });
    ...
    for (let j = 0; j < initialOptionIds.length; j++) {
      ...
      if (!keepOption) {
        yield axios.delete(
          `https://devinc-web-json-server-i27x.vercel.app/options/${initialOptionIds[j]}`
        );
      }
    }
    ...
    for (let i = 0; i < initialImageIds.length; i++) {
      ...
      if (!keepImage) {
        yield axios.delete(
          `https://devinc-web-json-server-i27x.vercel.app/images/${initialImageIds[i]}`
        );
      }
    }
