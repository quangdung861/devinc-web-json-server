...
function* getReviewListSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/reviews`, {
      params: {
        productId,
        _expand: "user",
        _sort: "id",
...
function* postReviewSaga(action) {
  try {
    ...
    const result = yield axios.post(
      `https://devinc-web-json-server-i27x.vercel.app/reviews`,
      action.payload
    );
    yield put({
...
function* patchReviewSaga(action) {
  try {
    ...
    const result = yield axios.patch(
      `https://devinc-web-json-server-i27x.vercel.app/reviews/${id}`,
      values
    );
    yield put({
