...
function* orderProductSaga(action) {
  try {
    ...
    const result = yield axios.post("https://devinc-web-json-server-i27x.vercel.app/orders", orderData);
    for (let i = 0; i < products.length; i++) {
      yield axios.post("https://devinc-web-json-server-i27x.vercel.app/orderProducts", {
    ...
    for (let i = 0; i < images.length; i++) {
      yield axios.post("https://devinc-web-json-server-i27x.vercel.app/orderProductImages", {
        orderId: result.data.id,
    ...
    yield axios.post("https://devinc-web-json-server-i27x.vercel.app/statusDetailOrders", {
      orderId: result.data.id,
...
function* guestOrderProductSaga(action) {
  try {
    ...
    const result = yield axios.post(
      "https://devinc-web-json-server-i27x.vercel.app/guestOrders",
      orderData
    );
    for (let i = 0; i < products.length; i++) {
      yield axios.post("https://devinc-web-json-server-i27x.vercel.app/guestOrderProducts", {
...
function* getOrderListSaga(action) {
  try {
    ...
    const result = yield axios.get("https://devinc-web-json-server-i27x.vercel.app/orders", {
      params: {
        ...(userId && {
          userId: userId,
        }),
...
function* getOrderDetailSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/orders/${orderId}`, {
      params: {
        _embed: ["orderProducts", "orderProductImages", "statusDetailOrders"],
      },
    });
...
function* getProductSoldSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/orderProducts`, {
      params: {
        productId,
      },
    });
...
function* handleOrderItemSaga(action) {
  try {
    const { id, params, messageOrder, paramsStatusDetailOrders, callback } =
      action.payload;
    const result = yield axios.patch(`https://devinc-web-json-server-i27x.vercel.app/orders/${id}`, {
      ...(params.orderStatus && {
    ...
    yield axios.patch(
      `https://devinc-web-json-server-i27x.vercel.app/statusDetailOrders/${paramsStatusDetailOrders.id}`,
      {
