...
function* getCityListSaga(action) {
  try {
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/cities`);
    yield put({
...
//   try {
//     const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/locations`);
//     yield put({
...
function* getDistrictListSaga(action) {
  try {
    ...
    const result = yield axios.get(
      `https://devinc-web-json-server-i27x.vercel.app/districts?parentcode=${cityCode}`
    );
...
function* getWardListSaga(action) {
  try {
    ...
    const result = yield axios.get(
      `https://devinc-web-json-server-i27x.vercel.app/wards?parentcode=${districtCode}`
    );
  ...
  try {
    ...
    if (defaultAddress === 1) {
      for (let i = 0; i < locationList.length; i++)
        yield axios.patch(
          `https://devinc-web-json-server-i27x.vercel.app/locations/${locationList[i].id}`,
          {
            default: 0,
          }
        );
    ...
    const result = yield axios.post(`https://devinc-web-json-server-i27x.vercel.app/locations`, {
      ...values,
...
function* deleteLocationSaga(action) {
  try {
    ...
    yield axios.delete(`https://devinc-web-json-server-i27x.vercel.app/locations/${locationId}`);
    yield put({
...
function* updateLocationSaga(action) {
  try {
    ...
    const result = yield axios.patch(
      `https://devinc-web-json-server-i27x.vercel.app/locations/${locationId}`,
      values
    );
    yield put({
...
function* setDefaultLocationSaga(action) {
  try {
    const { locationId, userId } = action.payload;
    const result = yield axios.get(
      `https://devinc-web-json-server-i27x.vercel.app/locations?userId=${userId}`
    );
    for (let i = 0; i < result.data.length; i++) {
      if (result.data[i].id === locationId) {
        yield axios.patch(
          `https://devinc-web-json-server-i27x.vercel.app/locations/${result.data[i].id}`,
          {
            default: 1,
          }
        );
