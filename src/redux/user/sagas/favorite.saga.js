...
function* favoriteProductSaga(action) {
  try {
    ...
    const result = yield axios.post(
      `https://devinc-web-json-server-i27x.vercel.app/favorites`,
      action.payload
    );
    yield put({
...
function* unFavoriteProductSaga(action) {
  try {
    ...
    yield axios.delete(`https://devinc-web-json-server-i27x.vercel.app/favorites/${id}`);
    yield put({
...
function* getfavoriteListSaga(action) {
  try {
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/favorites`);
    yield put({
