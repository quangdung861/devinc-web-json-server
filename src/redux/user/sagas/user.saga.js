...
function* registerSaga(action) {
  try {
    ...
    yield axios.post("https://devinc-web-json-server-i27x.vercel.app/register", data);
...
function* loginSaga(action) {
  try {
    const { data, callback } = action.payload;
    const result = yield axios.post("https://devinc-web-json-server-i27x.vercel.app/login", data);
...
function* getUserInfoSaga(action) {
  try {
    ...
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/users/${id}`, {
      params: {
        _embed: "locations",
      },
    });
...
function* updateUserInfoSaga(action) {
  try {
    ...
    yield axios.patch(`https://devinc-web-json-server-i27x.vercel.app/users/${userId}`, values);
    yield put({
