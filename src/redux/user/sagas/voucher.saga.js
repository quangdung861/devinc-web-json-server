...
function* getVoucherListSaga(action) {
  try {
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/vouchers`);
    yield put({
...
function* getVoucherShipListSaga(action) {
  try {
    const result = yield axios.get(`https://devinc-web-json-server-i27x.vercel.app/shipvouchers`);
    yield put({
